 ...
 * 
 * Usage: node simple-yield-dao-launcher.js --capital 10000
 */
...
class YieldDAOLauncher {
  ...
  async createVote(proposal) {
    ...
    console.log(`\nVote created: ID ${mockSigned.id}, options ${proposal.options.join('/')}`);
  ...
  async tallyVote(voteId) {
    ...
    // Get sentiment from X (mocked)
    const sentiment = { score: 0.85 };
  ...
  async monteCarloSim(params, simCount = 1000) {
    ...
    for (let i = 0; i < simCount; i++) {
      ...
      const simAllocation = this.simulateDynamicAllocate(
        this.capital, 
        pools, 
        simVol, 
        params.rlusdWeight
    ...
    for (const y of yields) {
      sumSquaredDiff += Math.pow((y * 100) - meanYield, 2);
    }
    const stdDev = Math.sqrt(sumSquaredDiff / yields.length);
    
    // Calculate min, max, and success rate
    const yieldValues = yields.map(y => y * 100);
   ...
   */
  simulateDynamicAllocate(capital, pools, vol, rlusdWeight) {
  ...
  async mintYieldNFT(results, winner) {
    ...
    console.log(`\nNFT Minted: ID ${mockSigned.id} with metadata {yield: ${results.meanYield.toFixed(2)}, winner: '${winner}'}`);
...
async function main() {
  try {
    ...
    const launcher = new YieldDAOLauncher(capital);
